...
function convertNumberToKformat(digit) {
    var num = digit;
    var numLength = num.toString().length;
    if (numLength > 3 && numLength < 7) { // 1000-999999
...
function generateVideoPlayerWrapper(obj) {
    ...
    var videoActionDivIcons = document.createElement("div");
    var heartIcon = document.createElement("i");
    var commentIcon = document.createElement("i");
    var bookmarkIcon = document.createElement("i");

    mainDiv.setAttribute("class", "playlist-card");
    videoActionDiv.setAttribute("id", "video-actions");
    viewsCount.setAttribute("id", "views-count");
    videoTitle.setAttribute("id", "video-title");
    videoDescription.setAttribute("id", "video-description");
    heartIcon.setAttribute("class", "far fa-heart");
    commentIcon.setAttribute("class", "far fa-comment-alt");
    bookmarkIcon.setAttribute("class", "far fa-bookmark");

    viewsCountSpan.innerHTML = convertNumberToKformat(obj.views) + " views";
    videoTitle.innerHTML = obj.title;
    videoDescription.innerHTML = obj.description;

    videoActionDivIcons.appendChild(heartIcon);
    videoActionDivIcons.appendChild(commentIcon);
    videoActionDivIcons.appendChild(bookmarkIcon);

    viewsCount.appendChild(viewsCountSpan);

    videoActionDiv.appendChild(viewsCount);
    videoActionDiv.appendChild(videoActionDivIcons);

    mainDiv.appendChild(videoActionDiv);
    mainDiv.appendChild(videoTitle);
    mainDiv.appendChild(videoDescription);
    return mainDiv;
}
...
$(document).ready(function () {
    ...
    var playlist = playlistData.map(createPlaylistCard);
    $('#playlist-wrapper').html(playlist);
